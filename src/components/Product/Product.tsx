...
function Product({ product }: ProductProps) {
  ...
  return (
    <LazyMotion features={domAnimation}>
      ...
      >
        <div className='absolute top-[-40px] w-full justify-center items-center'><Messages action={action} /></div>
        <div className="w-[120px] h-[120px] lg:w-[40%] lg:h-[150px] bg-tertiary rounded-[20px] mr-2">
        ...
        <div className="w-[50%] lg:w-[60%]">
          <p className="mt-2 mb-2 font-medium">{name}</p>
          <p className="font-semibold">${price.toString()} /x{portion.toString()}</p>
        </div>
        ...
        {location.pathname.includes('shopping')
          ? (
            <button
              ...
              className="absolute bottom-4 right-4 bg-red-500 text-white font-medium px-1 py-1 rounded-[50%]"
            >
